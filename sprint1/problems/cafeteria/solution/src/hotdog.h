...
public:
    ...
    HotDog(int id, std::shared_ptr<Sausage> sausage, std::shared_ptr<Bread> bread)
        ...
        if (sausage_->GetCookDuration() < MIN_SAUSAGE_COOK_DURATION
            ...
            //auto time_temp = Clock::duration::duration_cast<chrono::microseconds>(sausage_->GetCookDuration());
            throw std::invalid_argument("Invalid sausage cook duration " + std::to_string(sausage_->GetCookDuration().count()));
        ...
        if (bread_->GetBakingDuration() < MIN_BREAD_COOK_DURATION
            ...
            //auto time_temp = Clock::duration::duration_cast<chrono::microseconds>(bread_->GetBakingDuration());
            throw std::invalid_argument("Invalid sausage cook duration " + std::to_string(bread_->GetBakingDuration().count()));
        } 
    }
