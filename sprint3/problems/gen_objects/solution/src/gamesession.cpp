...
void GameSession::AddLoot(int num) {
    ...
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> distr(0, gamemap_ptr_->GetLootDescription().size());
    std::uniform_int_distribution<> road_distr(0, gamemap_ptr_->GetRoads().size());
    for (auto i =0; i < num; ++i) {
        ...
        if (road_ptr->IsHorizontal()) {
            std::uniform_int_distribution<> road_coor(0, road_ptr->GetEnd().x);
            loot.y = road_ptr->GetStart().y;
            loot.x = road_coor(gen);
        } else {
            std::uniform_int_distribution<> road_coor(0, road_ptr->GetEnd().y);
