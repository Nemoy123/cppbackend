...
//#endif
        ...
        WHEN("it is turned on first time") {  // Когда его включают в первый раз,
            ...
            AND_THEN("it can select channel from 1 to 99") {
                ...
                for (int n=0; n<10; ++n) {
                    try{ 
                        const int channel = distr(gen);
                        const int next_channel = distr(gen);
                        tv.SelectChannel(channel);
                        tv.SelectChannel(next_channel);
                        CHECK(tv.GetChannel().value() == next_channel);
                        tv.SelectLastViewedChannel ();
                        CHECK(tv.GetChannel().value() == channel);
                        tv.SelectLastViewedChannel ();
                        CHECK(tv.GetChannel().value() == next_channel);
                        tv.SelectLastViewedChannel ();
                        CHECK(tv.GetChannel().value() == channel);
