...
private:
    ...
    [[nodiscard]] bool SelectChannel(std::istream& input, std::ostream& output) const {
        ...
        if (auto temp = CheckChannel(input, output)) {
            try {
                tv_.SelectChannel(temp.value());
                return true;
            } 
    ...
    std::optional<int> CheckChannel(std::istream& input, std::ostream& output) const { 
        std::string data;
        input >> data;
        //std::string num_str = data.substr(14);
        if (isInteger(data)) {
