...
void RequestHandler::ApiHandler (std::string_view target, FuncMakeResponseString& text_response, FuncRequest& req, FuncSend& send) {
    if (target == "/api/v1/maps") {
                ...
                if (req.method_string() != "POST") { 
                    ...
                    res.insert (http::field::cache_control,"no-cache");
                    send(std::move(res));
                    return;
                ...
                response_message["authToken"] = new_player.token_string_;
                response_message["playerId"] = new_player.id_dog_;
                ...
                if (req.method_string() != "GET" && req.method_string() != "HEAD") { 
                    ...
                    res.result(http::status::method_not_allowed);
                    res.set(http::field::content_type, "application/json");
                    res.insert (http::field::allow, "GET, HEAD");
                    res.body() = json::serialize(obj);
                    res.content_length(res.body().size());
                    res.insert (http::field::cache_control,"no-cache");
                    send(std::move(res));
                ...
                if (tok != req.end()) {
                    ...
                    if (token.starts_with ("Bearer ") && token.size() > 7) {
                        ...
                        if (ptr_player == nullptr) {
                            ...
                            response_message["code"] = "unknownToken";
                            response_message["message"] = "Player token has not been found";
                        ...
                        for (const auto& name : game_.FindPlayersOnMap (map_id)) {
                            response_message[std::to_string(count)] = name;
                            ++count;
                        }
                        StringResponse res;
                        res.version(11);  // HTTP/1.1
                ...
                json::object response_message;
                response_message["code"] = "invalidToken";
                response_message["message"] = "Authorization header is missing";
