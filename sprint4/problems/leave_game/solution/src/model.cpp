...
const std::map <util::UUIDType, std::unique_ptr<Loot>>& Map::GetLootList () const  {
    return loot_list_;
}
std::map <util::UUIDType, std::unique_ptr<Loot>>& Map::GetLootList () {
    return loot_list_;
}
...
bool Map::operator==(const Map& other) const {
    if (       id_ == other.id_ 
        ...
        {   
            for (   auto i = loot_list_.begin(), t = other.loot_list_.begin(); 
                    i != loot_list_.end(), t != other.loot_list_.end();
                    ++i, ++t
                ) {
                if (i->first != t->first) return false;
...
void Map::AddLoot(int num) {
    ...
    for (auto i =0; i < num; ++i) {
        ...
        util::UUIDType id {util::NewUUID()};
        loot->id = id;
        loot_list_.insert ({id, std::move(loot)});
