...
Game LoadGame(Strand& strand, const std::string& res, Base& base) {

    Game game (strand, base);
...
Game LoadGame(Strand& strand, const std::filesystem::path& json_path, Base& base) {
    std::string temp{};
    std::string res{};
    std::fstream in(json_path);
    if (!in) {
    ...
    return LoadGame (strand, res, base);
}
