...
SCENARIO( "Game serialize") { 
    GIVEN("a game") { 
       ...
       const unsigned num_threads = std::thread::hardware_concurrency();
        ...
        std::string url_base {"postgres://postgres:Mys3Cr3t@localhost:30432/postgres"};
        Base base (num_threads, std::move(url_base));

        Game game = json_loader::LoadGame(api_strand, config, base);
        bool randomize = true;
